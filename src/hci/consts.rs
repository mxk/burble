...
/// Macro-generated company name look-up table. This representation saves ~3 KB
...
pub const fn company_name(id: u16) -> Option<&'static str> {
    let i = match id.checked_sub(CompanyName::MIN) {
        Some(0) => return Some(CompanyName::FIRST),
        Some(i) if (i as usize) < CompanyName::IDX.len() => i as usize,
        _ => return None,
    };
    let off = CompanyName::IDX[i - 1] as usize;
    // SAFETY: `TAB[IDX[i - 1]..IDX[i]]` contains a valid UTF-8 string
    Some(unsafe {
        std::str::from_utf8_unchecked(std::slice::from_raw_parts(
            CompanyName::TAB.as_ptr().add(off),
            CompanyName::IDX[i] as usize - off,
        ))
    })
}

macro_rules! company_names {
...
mod tests {
    ...
    #[test]
    fn company_names() {
        assert_eq!(company_name(u16::MIN), None);
        assert_eq!(company_name(CompanyName::MIN - 1), None);
        assert_eq!(company_name(CompanyName::MIN), Some(CompanyName::FIRST));
        assert_eq!(company_name(0xFCCC), Some("WiFi Alliance"));
        assert_eq!(company_name(CompanyName::MAX), Some("GN Netcom"));
        assert_eq!(company_name(CompanyName::MAX + 1), None);
        assert_eq!(company_name(u16::MAX), None);
    }
}
